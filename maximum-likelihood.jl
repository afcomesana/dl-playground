...
# ╔═╡ 9a41eeb6-315d-11ee-3b59-831a27ba6f72
begin
...
# ╔═╡ e2d97511-5a9e-49d7-8c61-78659125582c
begin
	# ####################
...
# ╔═╡ 2b2fa244-42f2-4755-98b3-a291d1bb92ed
polynomial_model(Θ::Vector{<:Real}) = f(x) = mapreduce(iter -> iter[2]*x^(iter[1] - 1), +, enumerate(Θ))

# ╔═╡ 6aa93085-01ab-491f-b66b-9556bd759efb
begin
	x_axis = Vector(-10:0.01:10)
	model = polynomial_model([5, 2, .5])
	plot = Plots.scatter(x_coords, y_coords)
	Plots.plot!(plot, x_axis, map(x -> model(x), x_axis))
end
...
julia_version = "1.8.3"
manifest_format = "2.0"
project_hash = "314133894538a32d8c8313578373e34ca81d8ee6"
