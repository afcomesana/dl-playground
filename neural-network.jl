...
# ╔═╡ 937c837e-470a-11ee-2d84-edeb61161ab4
begin
	import Flux
	import Zygote
end

# ╔═╡ f19dfccb-bf9f-4432-8d3b-ee0152fb101f
begin
	f(x) = 3x^2 + 2x + 1
	df(x) = Flux.gradient(f, x)[1]
	df2(x) = Flux.gradient(df, x)[1]
end

# ╔═╡ ac32ca0f-ab2b-43c0-8e47-52df80bd2e6e
begin
	g(x, y) = sum((x .- y).^2)
	Zygote.gradient(g, [0, 1], [2, 3])
end

# ╔═╡ 33c8b281-754e-4bfb-a0d8-0627bfd2879f
begin
	nt = (a = [2, 1], b = [2, 0], c = tanh)
	h(x::NamedTuple) = sum(abs2, x.a .- x.b)
	h(nt)
end

# ╔═╡ 0b6ddfb5-bf40-4787-8fe5-4e10fcf9f6d4
dh_nt = Flux.gradient(h, nt)[1]

# ╔═╡ b05bd09e-d315-4245-876a-7f46f70538e9
Flux.gradient((x, y) -> sum(abs2, x.a ./y .- x.b), nt, [1, 2])

# ╔═╡ 0ba64e53-4d54-49fc-8df4-cc67c6dee2aa
Flux.gradient(nt, [1, 2]) do x, y
	z = x.a ./ y
		sum(abs2, z .- x.b)
end

# ╔═╡ 91395a59-a056-4cd8-962e-cd4c2fef8eae
Flux.withgradient(h, nt)

# ╔═╡ f4ab3443-ab8a-460f-b884-ad0d8970decc
begin
	W = rand(2, 5)
	b = rand(2)
	X_train = rand(5)
	...
	for epoch in 1:100
		δmp = Zygote.gradient(params -> sum(abs2.((params.W*X_train .+ params.b) .- Y_train)), mp)[1]

		mp.W .-= η*δmp.W
		mp.b .-= η*δmp.b
		println(sum(abs2.((mp.W*X_train .+ mp.b) .- Y_train)))
	end
end
...
julia_version = "1.9.2"
manifest_format = "2.0"
project_hash = "63082bf9a9c174aa6fe619ea71569b04076a2147"
